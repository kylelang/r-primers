...
Let's apply `pivot_wider()` to a real world inquiry. The plot below visualizes an aspect of the `babynames` data set from the **babynames** package.
...
You've completed the Week 1 In-Class Exercises. Don't worry if you're still 
struggling with some concepts or feel that you haven't mastered everything 
covered in these tutorials: that's normal. If you managed to complete all of
these tutorial exercises, you should have enough familiarity R to hit the ground
running when we start using R to estimate statistical models (which we'll start
doing next week). Mastery will come through practice and repitition. 

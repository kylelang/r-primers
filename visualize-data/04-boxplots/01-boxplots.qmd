...
To make a boxplot with {ggplot2}, add `geom_boxplot()` to the [ggplot2 template](/basics/01-visualization-basics/01-general-ideas.qmd). For example, the code below uses boxplots to display the relationship between the `class` and `hwy` variables in the `mpg` dataset, which comes with {ggplot2}. 

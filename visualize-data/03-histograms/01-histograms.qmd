...
To make a histogram with {ggplot2}, add `geom_histogram()` to the [ggplot2 template](/basics/01-visualization-basics/01-general-ideas.qmd). For example, the code below plots a histogram of the `carat` variable in the `diamonds` dataset, which comes with {ggplot2}.
